...
plt.figure(figsize = (len(psf_stars), len(psf_stars)))
...
plt.xlabel('Actual Counts/COADD', fontsize = len(psf_stars))
plt.ylabel('Theoretical Counts/COADD', fontsize = len(psf_stars))
plt.title('{0} (Frame {2}) in {1}'.format(epoch, stf_ver, raw_frame), fontsize = len(psf_stars))
plt.legend()
